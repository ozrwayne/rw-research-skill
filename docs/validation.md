# RW Research Skill v0.11.0 验证记录
...
- 对外入口：4 个。
- License：Apache-2.0。
- 542 条知识原子、171 条公理、144 个案例和反例、161 条行为合同。
- 21 个 Skill 的静态自检通过。
- 仓库版本、入口 manifest、目录、公开计数和 Skill 接续检查通过。
- 结构化 Skill 交接检查覆盖 `cases.md` 的“下一步”与行为合同的 `expected_next`，未知目标会阻断仓库检查。
- 公共内容检查覆盖 Skill、文档和跨模型评测记录。542 条知识原子、144 个案例和 161 条行为合同的隐私检查失败项为 0。
- 知识原子不保留原始个人材料字段；案例和行为合同标记为合成输入。
- `rw-citation-audit` 独立结构检查通过。
- 确定性 Python 工具测试见仓库 `tests/` 和 CI 记录。

`rw-research-lab-router` 的工具登记表读取器是可选输入。它只标记登记日期的 `CURRENT`、`STALE` 或 `UNKNOWN`，不把登记表当成工具当前可用性的证明。
...
- 161 条行为合同没有被全部调用执行。跨模型矩阵执行的是另存的 8 个合成任务。
