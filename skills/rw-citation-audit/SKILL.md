...
`scripts/citation_audit.py` 是本地作者—年份语法预检，不查询 DOI、作者身份或出版元数据。输出 `structural_status` 表示已实现规则的匹配结果；总 `status` 在缺身份核验时至少为 REVIEW。正则未覆盖的引用形式应人工检查，不把无报错当成全文合格。文稿 SHA-256 来自本次解析的同一份字节。`--output` 不得等于输入，已有输出需 `--force`。
