...
2. 为每项研究建立 study_id、每份报告建立 report_id，并连接原文或可核验标识符；核对多报告关联和样本重叠，记录依据及未知状态。
...
- 证据图先固定节点定义，再建立关系。
- 研究结论和当前综合判断要分开保存。
- 研究、报告和效应记录分别计数；同一 study_id 或重叠样本不能在同一综合中被当作相互独立的证据。不同结局或时间点保留各自记录，合并前说明选取规则或依赖结构处理；重叠未知时不做独立性假设。
