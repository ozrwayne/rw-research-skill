...
def validate_display_items(data: dict[str, Any], errors: list[str]) -> None:
    ...
    if policy.get("numbering_policy") not in DISPLAY_NUMBERING_POLICIES:
    ...
    for index, item in enumerate(items):
        ...
        kind = item.get("kind")
        if kind not in DISPLAY_ITEM_KINDS:
        ...
        expected_reference = f"{kind.title()} {number}" if isinstance(number, int) else ""
        ...
        if not isinstance(upload_order, int) or upload_order < 1:
...
def display_preflight(data: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    ...
    main_path = Path(main_path_value)
...
def validate_proof(portal: dict[str, Any], errors: list[str]) -> None:
    ...
    if proof.get("text_check") not in PROOF_TEXT_CHECKS:
        errors.append("portal.proof.text_check is invalid")
    if proof.get("visual_check") not in PROOF_VISUAL_CHECKS:
        errors.append("portal.proof.visual_check is invalid")
    if proof.get("agent_observation") not in AGENT_OBSERVATIONS:
...
def command_preflight_display_items(args: argparse.Namespace) -> int:
    try:
        errors, report = display_preflight(load(Path(args.path)))
    except (OSError, json.JSONDecodeError, ValueError) as exc:
        print(f"invalid submission packet: {exc}")
        return 2
    if errors:
