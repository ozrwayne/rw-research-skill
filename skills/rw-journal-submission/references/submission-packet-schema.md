...
- JSON 加载拒绝重复键及非有限数字，写入同样禁止非有限数字。v2 必须包含 `final_submission`。非 `submitted` 状态的 evidence 为 `null`；平台回执至少保留非空稿件编号或回执引用。用户确认仍单独记录为 `user_confirmation`，不假称 Agent 已观察。
- `agent_verified` 校样要求 `observed`、检查时间和证据来源；`human_confirmed` 要求人类确认来源及检查时间，并保留 `not_observed`。`reviewed` 要求文字检查通过或不可用，以及已完成的视觉核对；文字失败不写成 reviewed。
- `complete` 门户步骤不得同时保留错误，同一资料包不得含重名步骤。
- 退回事件的解决时间不得早于退回时间，resolved 必须提供本次保存修复的非空 evidence；已解决事件不原地重写，新的退回建立新 ID。
- 更新门户、校样、退回和最终结果时，审计记录保存 `previous` 快照，保留旧证据。
- 展示项预检要求主文、展示项和对应材料均为 confirmed，核对声明的主文及独立文件 hash，并拒绝把同一文件、主文别名或硬链接作为独立展示项。相对路径以资料包所在目录为基准。
- DOCX 按段落拼接格式 run，包含旧式绘图检查；损坏 XML 返回错误。主 XML 上限为 32 MiB，超限交给人工或其他明确获准工具检查。
- CLI 写入使用同目录 `.lock` 文件协调同一 CLI 的并发调用，并原子保存。外部手工编辑和直接函数调用不受该锁保护。状态文件及锁文件不接受符号链接路径。
- `validate` 是结构与内部一致性检查，不是“可提交”门。资料包为空也可作为草稿合法存在；人类仍须核对当前期刊要求、作者与披露、实际文件版本、校样和门户状态。脚本不读取原始确认记录、不判断证据真假，也不进行最终提交。
