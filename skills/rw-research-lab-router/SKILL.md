...
7. 用户提供 Research Lab 工具登记表时，可用 `scripts/read_tool_registry.py` 读取版本、许可证、运行环境、测试日期和 freshness 状态；没有登记表时保持独立运行。
...
4. 检查许可证、认证、费用、限流、维护状态、数据外发和可复现性。
   如果有工具登记表，先把登记值与源码或运行结果分开记录，不把登记表当成当前可用性的证明。
...
- 工具登记表是可选输入；登记版本、测试日期和源码状态不一致时，标记为 `STALE` 或 `REVIEW`，不静默覆盖。
